...
function Market() {
  ...
  const handlePlaceBet = (betId) => {
    ...
    const updatedBets = openedBets.map(bet => {
      if (bet.id === betId) {
        const updatedIndividualBets = [...(bet.individualBets || []), newIndividualBet];
        const higherBets = updatedIndividualBets.filter(b => b.prediction === 'higher');
        const lowerBets = updatedIndividualBets.filter(b => b.prediction === 'lower');
    ...
    const higherTotal = updatedBets.find(bet => bet.id === betId)?.higherTotal || 0;
    const lowerTotal = updatedBets.find(bet => bet.id === betId)?.lowerTotal || 0;
    const oppositeTotal = betPrediction === 'higher' ? lowerTotal : higherTotal;
    const potentialPayout = oppositeTotal > 0 ? (betAmount / (betPrediction === 'higher' ? higherTotal : lowerTotal)) * oppositeTotal : 0;
  ...
  const getBetStatusText = (status) => {
    switch (status) {
      ...
      case 'revealed': return 'Bet Ended';
      case 'closed': return 'Bet Closed';
      ...
      <div className="bets-grid">
        {openedBets.length > 0 ? (
              ...
              <div className="bet-header">
                <div className="bet-info">
                  <div className="bet-opener">
                    <img src={bet.openerAvatar} alt={bet.openerUsername} className="opener-avatar" />
                    <div>
                      <h3>{bet.openerUsername}</h3>
                      <p className="test-info">{bet.subject} - {bet.testName}</p>
                    </div>
                  </div>
                  <div className="bet-details">
                    ...
                    <div className="pot-display">
                      ...
                      <span className="pot-value">${bet.totalPot.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              ...
              {bet.status === 'revealed' && (
                <div className="bet-results">
                  ...
                  <div className="result-summary">
                    <p><strong>Actual Score:</strong> {bet.actualScore}%</p>
                    <p><strong>Threshold:</strong> {bet.threshold}%</p>
                    <p><strong>Winning Side:</strong> {bet.winningSide}</p>
                      ...
                      <div className="payout-summary">
                        ...
                        <ul>
                          {bet.payouts.map((payout, index) => (
                            <li key={index}>
                              {payout.username}: ${payout.betAmount} → ${payout.totalWinnings.toFixed(2)} (+${payout.payout.toFixed(2)})
                            </li>
                          ))}
                        </ul>
                      </div>
                      ...
                      <div className="bets-list">
                        <div className="bet-summary">
                          <div className="bet-side">
                            <span className="side-label">Over: ${(bet.higherTotal || 0).toFixed(2)}</span>
                            <span className="side-count">({(bet.individualBets || []).filter(b => b.prediction === 'higher').length} bets)</span>
                          </div>
                          <div className="bet-side">
                            <span className="side-label">Under: ${(bet.lowerTotal || 0).toFixed(2)}</span>
                            <span className="side-count">({(bet.individualBets || []).filter(b => b.prediction === 'lower').length} bets)</span>
                          </div>
                        </div>
                    <div className="total-pot">
                      <span className="pot-label">Total Pot: ${(bet.totalPot || 0).toFixed(2)}</span>
                    </div>
                  </div>
                ) : (
          ...
          <div className="no-bets-message">
            ...
            <p>Be the first to open a bet! Go to the "Open Bet" tab to create one.</p>
          </div>
        )}
      </div>
            ...
            <div className="bet-form">
              <div className="form-group">
                <label>Prediction:</label>
                  ...
                  <div className="bet-summary">
                    <p><strong>Your Wallet:</strong> ${currentUser.wallet.toFixed(2)}</p>
                    <p><strong>Threshold:</strong> {selectedBet ? openedBets.find(bet => bet.id === selectedBet)?.threshold : 85}%</p>
                    <p><strong>Your Prediction:</strong> {betPrediction}</p>
                    <p><strong>Your Bet Amount:</strong> ${betAmount}</p>
                {(() => {
                  const betData = openedBets.find(bet => bet.id === selectedBet);
                  if (!betData) return null;
                  const higherTotal = betData.higherTotal || 0;
                  const lowerTotal = betData.lowerTotal || 0;
                  const oppositeTotal = betPrediction === 'higher' ? lowerTotal : higherTotal;
                  const yourSideTotal = betPrediction === 'higher' ? higherTotal : lowerTotal;
                  const potentialPayout = oppositeTotal > 0 ? (betAmount / (yourSideTotal + betAmount)) * oppositeTotal : 0;
                  
                  return (
                    <>
                      <p><strong>Current {betPrediction} total:</strong> ${yourSideTotal.toFixed(2)}</p>
                      <p><strong>Opposite side total:</strong> ${oppositeTotal.toFixed(2)}</p>
                      <p><strong>Potential Payout:</strong> ${potentialPayout.toFixed(2)}</p>
                    </>
                  );
                })()}
              </div>
