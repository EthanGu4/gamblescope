...
function Wallet() {
  ...
  useEffect(() => {
    const savedTransactions = localStorage.getItem(
      `gamblescope_transactions_${currentUser.id}`,
    );
  ...
  const handleWithdraw = async (e) => {
    ...
    if (result.success) {
      ...
      setMessage({
        type: "success",
        text: `Successfully withdrew $${withdrawAmount}`,
      });
      setWithdrawAmount("");
    } else {
      setMessage({ type: "error", text: result.error });
      ...
      <div className="wallet-content">
        ...
        <div className="quick-deposit">
          ...
          <div className="quick-buttons">
            <button onClick={() => quickDeposit(10)} className="quick-btn">
              $10
            </button>
            <button onClick={() => quickDeposit(25)} className="quick-btn">
              $25
            </button>
            <button onClick={() => quickDeposit(50)} className="quick-btn">
              $50
            </button>
            <button onClick={() => quickDeposit(100)} className="quick-btn">
              $100
            </button>
          </div>
        </div>
            ...
            <div className="history-list">
              {transactions.map((transaction) => (
                <div key={transaction.id} className="history-item">
                  ...
                  <span
                    className={`history-amount ${transaction.type === "deposit" || transaction.type === "win" ? "positive" : "negative"}`}
                  >
                    {transaction.type === "deposit" ||
                    transaction.type === "win"
                      ? "+"
