...
function AdminPanel() {
  ...
  const handleClearAllBets = () => {
    if (
      window.confirm(
        "Are you sure you want to clear ALL bets? This action cannot be undone.",
      )
    ) {
  ...
  const handleClearUserBets = (userId, username) => {
    if (
      window.confirm(`Are you sure you want to clear all bets for ${username}?`)
    ) {
  ...
  const handleClearIndividualBet = (betId, betTitle) => {
    if (
      window.confirm(`Are you sure you want to clear the bet "${betTitle}"?`)
    ) {
  ...
  const handleGenerateMockData = () => {
    if (
      window.confirm(
        "This will generate 10 demo bets with mock betting activity. Continue?",
      )
    ) {
      ...
      if (score >= 0 && score <= 100) {
        ...
        payouts.forEach((payout) => {
          try {
            ...
            console.log(
              `Admin payout: ${payout.username} received $${payout.totalWinnings.toFixed(2)}`,
            );
          } catch (error) {
            console.log(
        ...
        alert(
          `Score revealed! Actual score: ${score}% (${winningSide} than ${bet.threshold}%)${payoutSummary}`,
        );
      } else {
        ...
        <div className="admin-section">
          ...
          <div className="bet-stats">
            ...
            <div className="stat-card">
              <h4>Revealed Bets</h4>
              <p className="stat-number">
                {allBets.filter((bet) => bet.status === "revealed").length}
              </p>
            </div>
          </div>

          <div className="admin-actions">
            <button className="danger-button" onClick={handleClearAllBets}>
              Clear All Bets
            </button>
            <button className="demo-button" onClick={handleGenerateMockData}>
              Generate Demo Data
            </button>
          </div>
        </div>
                ...
                <div key={user.id} className="user-card">
                  ...
                  <div className="user-actions">
                    <button
                      ...
                      onClick={() =>
                        handleClearUserBets(user.id, user.username)
                      }
                    >
                      Clear Bets
                    </button>
              ...
              {allBets.slice(0, 10).map((bet) => (
                <div key={bet.id} className="bet-item">
                  <div className="bet-info">
                    ...
                    <p>
                      Threshold: {bet.threshold}% • Status: {bet.status}
                    </p>
