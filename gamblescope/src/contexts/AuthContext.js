...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const adminExists = parsedUsers.some(
      (user) => user.username === "admin" && user.isAdmin,
    );
    if (!adminExists) {
      const initialAdmin = {
        id: "admin-001",
  ...
  const updateUserWallet = (userId, amount) => {
    setUsers((prev) =>
      prev.map((user) =>
        user.id === userId ? { ...user, wallet: user.wallet + amount } : user,
      ),
    );
  ...
  const clearUserBets = (userId) => {
    ...
    return {
      success: true,
      message: `Bets for user ${userId} cleared successfully`,
    };
  };
  ...
  const clearIndividualBet = (betId) => {
    ...
    if (betToDelete.individualBets && betToDelete.individualBets.length > 0) {
      betToDelete.individualBets.forEach((individualBet) => {
        try {
          ...
          console.log(
            `Returned $${individualBet.amount} to ${individualBet.username} (bet closed by admin)`,
          );
        } catch (error) {
          console.log(
            `Could not return bet amount to user ${individualBet.username} - user not found`,
          );
        }
      });
    }
    ...
    return {
      success: true,
      message: `Bet "${betToDelete.subject} - ${betToDelete.testName}" cleared successfully. All bettors received refunds.`,
    };
  };
